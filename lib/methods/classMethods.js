Meteor.methods({
    createClass: function(classData){
    	if(Meteor.isServer){
		    ...
		    var fields = {
		    	...
		    	"categories": ["knowledge", "thinking", "application", "communication"],
	...
	deleteClass: function(classData){
		if(Meteor.isServer){
			if(!Classes.findOne({"_id": classData.classId, "authorId": Meteor.userId()})){
				throw new Meteor.Error("Error: Failed to delte class")
			}

			Classes.remove({"_id": classData.classId, "authorId": Meteor.userId()})
		}
	}
})